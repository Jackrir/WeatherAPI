// <auto-generated />
using System;
using DataLayer;
using Microsoft.EntityFrameworkCore;
using Microsoft.EntityFrameworkCore.Infrastructure;
using Microsoft.EntityFrameworkCore.Metadata;
using Microsoft.EntityFrameworkCore.Migrations;
using Microsoft.EntityFrameworkCore.Storage.ValueConversion;

namespace DataLayer.Migrations
{
    [DbContext(typeof(AppDBContext))]
    [Migration("20210719202654_AddStartData")]
    partial class AddStartData
    {
        protected override void BuildTargetModel(ModelBuilder modelBuilder)
        {
#pragma warning disable 612, 618
            modelBuilder
                .HasAnnotation("Relational:MaxIdentifierLength", 128)
                .HasAnnotation("ProductVersion", "5.0.8")
                .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

            modelBuilder.Entity("DataLayer.Entities.City", b =>
                {
                    b.Property<string>("Name")
                        .HasColumnType("nvarchar(450)");

                    b.Property<double>("AverageTemperature")
                        .HasColumnType("float");

                    b.Property<double>("CurrentTemperature")
                        .HasColumnType("float");

                    b.Property<double>("MaxTemperature")
                        .HasColumnType("float");

                    b.Property<double>("MinTemperature")
                        .HasColumnType("float");

                    b.HasKey("Name");

                    b.ToTable("Cities");

                    b.HasData(
                        new
                        {
                            Name = "Kharkiv",
                            AverageTemperature = 24.5,
                            CurrentTemperature = 23.0,
                            MaxTemperature = 33.0,
                            MinTemperature = 15.0
                        },
                        new
                        {
                            Name = "Kiev",
                            AverageTemperature = 29.0,
                            CurrentTemperature = 29.0,
                            MaxTemperature = 36.0,
                            MinTemperature = 23.0
                        },
                        new
                        {
                            Name = "Lviv",
                            AverageTemperature = 25.0,
                            CurrentTemperature = 28.0,
                            MaxTemperature = 38.0,
                            MinTemperature = 23.0
                        });
                });

            modelBuilder.Entity("DataLayer.Entities.Measure", b =>
                {
                    b.Property<string>("CityName")
                        .HasColumnType("nvarchar(450)");

                    b.Property<DateTime>("Time")
                        .HasColumnType("datetime2");

                    b.Property<bool>("ArchiveStatus")
                        .HasColumnType("bit");

                    b.Property<double>("Temperature")
                        .HasColumnType("float");

                    b.HasKey("CityName", "Time");

                    b.ToTable("Measures");

                    b.HasData(
                        new
                        {
                            CityName = "Kharkiv",
                            Time = new DateTime(2021, 7, 19, 9, 0, 0, 441, DateTimeKind.Unspecified),
                            ArchiveStatus = false,
                            Temperature = 15.0
                        },
                        new
                        {
                            CityName = "Kharkiv",
                            Time = new DateTime(2021, 7, 19, 12, 0, 0, 441, DateTimeKind.Unspecified),
                            ArchiveStatus = false,
                            Temperature = 26.0
                        },
                        new
                        {
                            CityName = "Kharkiv",
                            Time = new DateTime(2021, 7, 19, 15, 0, 0, 441, DateTimeKind.Unspecified),
                            ArchiveStatus = false,
                            Temperature = 33.0
                        },
                        new
                        {
                            CityName = "Kharkiv",
                            Time = new DateTime(2021, 7, 19, 18, 0, 0, 441, DateTimeKind.Unspecified),
                            ArchiveStatus = false,
                            Temperature = 23.0
                        },
                        new
                        {
                            CityName = "Kharkiv",
                            Time = new DateTime(2021, 7, 19, 19, 0, 42, 676, DateTimeKind.Unspecified),
                            ArchiveStatus = true,
                            Temperature = 10.0
                        },
                        new
                        {
                            CityName = "Kharkiv",
                            Time = new DateTime(2021, 7, 19, 19, 10, 42, 676, DateTimeKind.Unspecified),
                            ArchiveStatus = true,
                            Temperature = 20.0
                        },
                        new
                        {
                            CityName = "Kiev",
                            Time = new DateTime(2021, 7, 19, 9, 0, 0, 441, DateTimeKind.Unspecified),
                            ArchiveStatus = false,
                            Temperature = 23.0
                        },
                        new
                        {
                            CityName = "Kiev",
                            Time = new DateTime(2021, 7, 19, 12, 0, 0, 441, DateTimeKind.Unspecified),
                            ArchiveStatus = false,
                            Temperature = 28.0
                        },
                        new
                        {
                            CityName = "Kiev",
                            Time = new DateTime(2021, 7, 19, 15, 0, 0, 441, DateTimeKind.Unspecified),
                            ArchiveStatus = false,
                            Temperature = 36.0
                        },
                        new
                        {
                            CityName = "Kiev",
                            Time = new DateTime(2021, 7, 19, 18, 0, 0, 441, DateTimeKind.Unspecified),
                            ArchiveStatus = false,
                            Temperature = 29.0
                        },
                        new
                        {
                            CityName = "Lviv",
                            Time = new DateTime(2021, 7, 19, 9, 0, 0, 441, DateTimeKind.Unspecified),
                            ArchiveStatus = false,
                            Temperature = 23.0
                        },
                        new
                        {
                            CityName = "Lviv",
                            Time = new DateTime(2021, 7, 19, 12, 0, 0, 441, DateTimeKind.Unspecified),
                            ArchiveStatus = false,
                            Temperature = 24.0
                        },
                        new
                        {
                            CityName = "Lviv",
                            Time = new DateTime(2021, 7, 19, 15, 0, 0, 441, DateTimeKind.Unspecified),
                            ArchiveStatus = false,
                            Temperature = 28.0
                        },
                        new
                        {
                            CityName = "Lviv",
                            Time = new DateTime(2021, 7, 19, 18, 0, 0, 441, DateTimeKind.Unspecified),
                            ArchiveStatus = true,
                            Temperature = 27.0
                        });
                });

            modelBuilder.Entity("DataLayer.Entities.Measure", b =>
                {
                    b.HasOne("DataLayer.Entities.City", "City")
                        .WithMany("Measures")
                        .HasForeignKey("CityName")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("City");
                });

            modelBuilder.Entity("DataLayer.Entities.City", b =>
                {
                    b.Navigation("Measures");
                });
#pragma warning restore 612, 618
        }
    }
}
